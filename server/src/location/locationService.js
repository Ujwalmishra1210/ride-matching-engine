...
async function getNearbyDrivers(lat,lng,radiusKm=5){
  ...
  for(const [driverId,distStr,[lngStr,latStr]] of results){
     ...
     if(driverState.status !== DRIVER_STATES.AVAILABLE){
        continue;
     }
...
async function updateDriverState(driverId,newState){
...
module.exports={updateDriverLocation,getNearbyDrivers,getDriverState,updateDriverState};